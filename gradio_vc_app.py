...
if VOICE_FILTERS_ENABLED:
    try:
        ...
        print("SUCCESS: Ghost filters loaded for Voice Conversion (with parameterized support)")
...
def generate(model, audio, target_voice_path):
    if model is None:
        model = ChatterboxVC.from_pretrained(DEVICE)
...
with gr.Blocks(title="Chatterbox Voice Conversion") as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            run_btn = gr.Button("Convert Voice", variant="primary")
    ...
    run_btn.click(
        fn=generate,
        inputs=[model_state, input_audio, target_voice],
